...
export default function CoinDetailClient({ locale, symbol }: Props) {
  ...
  const name = isKo ? coinMeta.nameKo : isZh ? coinMeta.nameZh : coinMeta.name;
  const premium = coin?.premium ?? null;

  const stats = coin ? [
    {
      label: isKo ? "업비트 (KRW)" : isZh ? "Upbit (KRW)" : "Upbit (KRW)",
    ...
    {
      label: isKo ? "김치 프리미엄" : isZh ? "泡菜溢价" : "Kimchi Premium",
      value: premium !== null ? formatPremium(premium) : "—",
      color: premium === null ? "text-[var(--fg-muted)]" : premium >= 0 ? "text-emerald-400" : "text-rose-400",
    },
    {
      label: isKo ? "24시간 변동" : isZh ? "24h 涨跌" : "24h Change",
      value: `${coin.change24h >= 0 ? "+" : ""}${coin.change24h.toFixed(2)}%`,
      color: coin.change24h >= 0 ? "text-emerald-400" : "text-rose-400",
    },
    {
      label: isKo ? "거래량 (24h)" : isZh ? "交易量 (24h)" : "Volume (24h)",
      value: formatVolume(coin.volume24h, locale),
      color: "text-[var(--fg-secondary)]",
    },
    {
      label: isKo ? "원/달러 환율" : isZh ? "美元/韩元" : "USD/KRW",
      value: `₩${exchangeRate.toLocaleString(locale === "zh" ? "zh-CN" : locale === "en" ? "en-US" : "ko-KR")}`,
      color: "text-[var(--fg-secondary)]",
    },
  ] : [];

  const tabs = [
    { key: "overview",  ko: "개요",    zh: "概览",    en: "Overview" },
    { key: "analysis",  ko: "심층 분석", zh: "深度分析", en: "Analysis" },
    { key: "sources",   ko: "출처",    zh: "来源",    en: "Sources"  },
  ] as const;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 space-y-6">
      ...
      >
        <ArrowLeft size={14} />
        {isKo ? "전체 코인 목록" : isZh ? "全部币种" : "All Coins"}
      </Link>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="flex border-b border-white/5">
          {tabs.map((tab) => (
            ...
            >
              {isKo ? tab.ko : isZh ? tab.zh : tab.en}
            </button>
          ))}
        </div>
          ...
          {!content ? (
            <div className="rounded-xl border border-dashed border-white/10 p-8 text-center">
              <p className="text-sm text-[var(--fg-muted)]">
                {isKo ? "분석 내용이 곧 추가됩니다." : isZh ? "分析内容即将添加。" : "Analysis coming soon."}
              </p>
            </div>
          ) : activeTab === "overview" ? (
              ...
              {content.type === "facts" && content.facts && (
                <div>
                  <h2 className="text-sm font-bold text-white mb-3">
                    {isKo ? "핵심 정보" : isZh ? "核心信息" : "Key Facts"}
                  </h2>
            ...
            <div>
              {content.sources && content.sources.length > 0 ? (
                ...
                <p className="text-sm text-[var(--fg-muted)] text-center py-6">
                  {isKo ? "참고 자료가 없습니다." : isZh ? "暂无参考资料。" : "No sources available."}
                </p>
              )}
            </div>
          )}
