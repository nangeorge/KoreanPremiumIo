...
const INFO: Record<string, InfoText> = {
  mvrv: {
    ...
    source: { label: "Glassnode — MVRV Ratio", url: "https://studio.glassnode.com/metrics?a=BTC&m=market.Mvrv" },
  },
  mvrvChart: {
    ...
    source: { label: "CoinMetrics Community API", url: "https://coinmetrics.io/community-network-data/" },
  },
  hashRate: {
    ...
    source: { label: "blockchain.com — Hash Rate", url: "https://www.blockchain.com/explorer/charts/hash-rate" },
  },
  activeAddr: {
    ...
    source: { label: "Glassnode — Active Addresses", url: "https://studio.glassnode.com/metrics?a=BTC&m=addresses.ActiveCount" },
  },
  txCount: {
    ...
    source: { label: "blockchain.com — Transactions", url: "https://www.blockchain.com/explorer/charts/n-transactions" },
  },
  fearGreed: {
    ko: "0~100 사이 값으로 시장 심리를 나타냅니다. 0~25 극도의 공포(매수 기회), 75~100 극도의 탐욕(과열 경고). 변동성·거래량·소셜미디어 등을 종합해 산출.",
    en: "0–100 index measuring market sentiment. 0–25 Extreme Fear (buying opportunity), 75–100 Extreme Greed (overheating warning). Computed from volatility, volume, social media, and more.",
    zh: "0~100的市场情绪指数。0~25极度恐慌（买入机会），75~100极度贪婪（过热警告）。综合波动性、交易量、社交媒体等计算。",
    source: { label: "Alternative.me — Fear & Greed Index", url: "https://alternative.me/crypto/fear-and-greed-index/" },
  },
  kimchiPremium: {
    ko: "국내 거래소(업비트) 가격이 해외 거래소 대비 얼마나 높은지 나타내는 수치. 양수면 한국 시장 과열, 음수면 할인 상태. 강세장에서 역사적 최고 50% 이상 기록.",
    en: "Percentage difference between Korean exchange (Upbit) and overseas exchange prices. Positive = Korean market premium, negative = discount. Historically peaked above 50% in bull markets.",
    zh: "韩国交易所（Upbit）与海外交易所价格差的百分比。正数=韩国溢价，负数=折价。牛市历史最高曾超过50%。",
    source: { label: "Upbit — BTC/KRW", url: "https://upbit.com/exchange?code=CRIX.UPBIT.KRW-BTC" },
  },
  vix: {
    ...
    source: { label: "TradingView — VIX (CBOE)", url: "https://www.tradingview.com/chart/?symbol=CBOE:VIX" },
  },
  fundingBtc: {
    ko: "무기한 선물에서 8시간마다 롱/숏 간 정산 비용. 양수(+)=롱 과열, 음수(-)=숏 과열. OKX 선물 기준.",
    ...
    source: { label: "OKX — Funding Rate History", url: "https://www.okx.com/trade-market/funding/swap" },
  },
  fundingEth: {
    ...
    source: { label: "OKX — ETH Funding Rate", url: "https://www.okx.com/trade-market/funding/swap" },
  },
  fundingAnnual: {
    ...
    source: { label: "OKX — Funding Rate", url: "https://www.okx.com/trade-market/funding/swap" },
  },
  fundingSentiment: {
  ...
  rsi: {
    ko: "RSI(14) — 과매수/과매도 지표. 70↑ 과매수(단기 조정 가능), 30↓ 과매도(반등 가능). Binance BTC-USDT 기준.",
    en: "RSI(14) — Overbought/oversold indicator. Above 70 = overbought (correction risk), below 30 = oversold (rebound potential). Source: Binance BTC-USDT.",
    zh: "RSI(14)——超买/超卖指标。高于70=超买（调整风险），低于30=超卖（反弹可能）。数据来源：Binance BTC-USDT。",
    source: { label: "Binance — BTC/USDT", url: "https://www.binance.com/en/trade/BTC_USDT" },
  },
  openInterest: {
    ko: "선물 시장 전체 미결제 포지션 규모. OI 급증=레버리지 과열, OI 급감=청산 연쇄 신호. Binance 무기한 선물 기준.",
    en: "Total open futures positions. Surging OI = leverage overheating, sharp drop = liquidation cascade risk. Source: Binance perpetuals.",
    zh: "期货市场未平仓头寸总量。OI急升=杠杆过热，急降=爆仓风险。数据来源：Binance永续合约。",
    source: { label: "Binance Futures — Open Interest", url: "https://www.binance.com/en/futures/BTC_USDT" },
  },
  longShort: {
    ...
    source: { label: "OKX — Long/Short Ratio", url: "https://www.okx.com/trade-market/position/swap" },
  },
  stablecoin: {
    ko: "전체 블록체인의 스테이블코인 총 발행량. 증가=크립토 시장 자금 유입, 감소=자금 이탈.",
    en: "Total stablecoin supply across all chains. Rising = capital inflows into crypto, falling = outflows.",
    zh: "全链稳定币总供应量。增加=资金流入加密市场，减少=资金外流。",
